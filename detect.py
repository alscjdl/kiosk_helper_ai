...
# OpenAI 클라이언트 초기화 (API 키 입력)
client = OpenAI(api_key="여기에 API 키 복붙해서 넣으면 됨 (" " 냅두기)")
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "단, 가이드 문장 안에는 오타(예: '다장')를 그대로 노출하지 말고 올바른 우리말 단어만 사용해.\n"
...
def generate_guide(detected, payment_buttons_text):
    ...
    # ⭐ [우선순위 1위] 화면에 메뉴 영역이나 카테고리 칸이 보이면 무조건 '메뉴 선택 단계' (라멘집 화면 완벽 방어)
    if "menu_area" in detected or "category_button" in detected:
        print("\n현재 단계: 메뉴 선택 단계")
        situation_context = "화면에 다양한 음식 메뉴들과 종류별 선택 칸이 나열되어 있어서, 먹고 싶은 음식을 손가락으로 골라 담는 단계"

    # 2. 장바구니와 버튼이 같이 있는 경우 (장바구니 확인 및 결제 단계)
    elif "payment_button" in detected and "cart_area" in detected:
    ...
    # 3. 버튼 글자 중에 '호출'이나 '호흡'만 달랑 들어온 경우 (직원 호출 단계)
    ...
    # 5. 버튼은 검출되었으나 결제/호출/메뉴판 흔적이 전혀 없는 경우 (맥도날드 첫 화면 매장/포장 단계)
